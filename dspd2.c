...
void allot_classrooms()
{
    ...
    for (int i = 0; i < num_courses; i++)
    {
        ...
        if (!found_classroom)
        {
            int least_num_students = courses[i].num_students;
            int num_least_courses = 0;
            for (int j = i + 1; j < num_courses && courses[j].slot == courses[i].slot; j++)
            {
                Student *student_ptr = courses[j].students;
                int num_students_j = 0;
                while (student_ptr != NULL)
                {
                    num_students_j++;
                    student_ptr = student_ptr->next;
                }
                if (num_students_j < least_num_students)
                {
                    least_num_students = num_students_j;
                    num_least_courses = 1;
                }
                else if (num_students_j == least_num_students)
                {
                    num_least_courses++;
                }
            }

            int num_courses_together = num_least_courses;
            while (num_courses_together > 0 && !found_classroom)
            {
                int least_num_students_together = courses[i].num_students;
                for (int j = i; j < i + num_courses_together; j++)
                {
                    Student *student_ptr = courses[j].students;
                    int num_students_j = 0;
                    while (student_ptr != NULL)
                    ...
                    if (num_students_j < least_num_students_together)
                    {
                        least_num_students_together = num_students_j;
                    }
                }

                int least_num_courses_together = 0;
                for (int j = i; j < i + num_courses_together; j++)
                {
                    Student *student_ptr = courses[j].students;
                    int num_students_j = 0;
                    ...
                    if (num_students_j == least_num_students_together)
                    {
                        least_num_courses_together++;
                    }
                }

                for (int j = 0; j < num_classrooms; j++)
                {
                    if (classrooms[j].capacity >= least_num_students_together)
                    {
                        int slot_available = 1;
                        for (int k = i - 1; k >= 0 && courses[k].slot == courses[i].slot; k--)
                        {
                            if (check_slot(courses[k].slot, classrooms[j].slots[i]))
                            {
                                slot_available = 0;
                                break;
                            }
                        ...
                        if (slot_available)
                        {
                            for (int k = i; k < i + num_courses_together; k++)
                            {
                                strcpy(courses[k].courses_room, classrooms[j].room_number);
                                classrooms[j].slots[i] = courses[i].slot;
                                printf("Room alloted %s for %s(Slot %d)\n", classrooms[j].room_number, courses[i].name, courses[i].slot);
                            }
                            classrooms[j].capacity -= least_num_students_together;
                            found_classroom = 1;
                            break;
                        }
                    }
                ...
                num_courses_together--;
            }
        }

        if (!found_classroom)
        {
            printf("No classroom available for %s in slot %d\n", courses[i].name, courses[i].slot);
        }
    }
}
...
// {
//     int slot = 1;
//     // Find all courses with the given slot
...
//                     printf("%s: Room %s, Row %d, Bench %d, Seat %d\n", current_student->name, room->room_number, row, bench, seat);
//                     assigned_students++;
//                     break;
//                 }
//             }
